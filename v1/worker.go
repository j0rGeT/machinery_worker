...
import (
	...
	"machinery_worker/v1/backends/amqp"
	"machinery_worker/v1/brokers/errs"
	"machinery_worker/v1/log"
	"machinery_worker/v1/retry"
	"machinery_worker/v1/tasks"
	"machinery_worker/v1/tracing"
...
func (worker *Worker) Process(signature *tasks.Signature) error {
	...
	sigType := signature.Name

	if sigType == "CreateVMCallback" {
		return worker.taskSucceeded(signature, results)
	} else {
		return worker.taskExecuting(signature, results)
	}
	//return worker.taskSucceeded(signature, results)
}
...
func (worker *Worker) taskExecuting(signature *tasks.Signature, taskResults []*tasks.TaskResult) error {
	// Update task state to SUCCESS
	if err := worker.server.GetBackend().SetStateExecuting(signature); err != nil {
